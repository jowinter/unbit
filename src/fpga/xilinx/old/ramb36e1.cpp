...
namespace unbit
{
	namespace old
	{
		namespace xilinx
		{
			...
			//
			// The mapping between bits in the bitstream and data/parity bits in the RAM can be easily
			// found by synthesizing an FPGA design with just a single block RAM and generating a
			// bitstream with logic location information. The mapping tables in this file were inferred
			// from the lofic location information produced by synthesis and implementation of such a
			// design (single_bram_top.v) for a Zynq XC7Z020 FPGA with Xilinx 2019.1 (or later) tools.
			//
			// Block RAM content is stored in a dedicated frame address range. Details on the frame
			// address range can be found in [Xilinx UG470; "Ch. 5 Configuration Details; Configuration
			// Registers; Frame Address Register"]. From the logic location information of our synthesis
			// expriment we can infer that block RAM content is stored in contiguous (bit) chunks.
			//
			// Excerpt from the logic location information produced by bitgen for the single_bram_top.v
			// design (on an XC7Z020 FPGA):
			//
			//  > ...
			//  > Bit 29857216 0x00c20000      0 Block=RAMB36_X0Y0 Ram=B:BIT0     <== First configuration frame of this RAM
			...
			//
			// The mapping of X/Y coordinates to the (start) byte offsets depends on the specific FPGA's
			// geometry. The structure inside the contiguous block for a RAM is identical for each
			// RAM. We store the internal mapping of RAM bits here; the start addresses of the config
			// blocks are stored along with the specific FPGA type, and are communicated as
			// bitstream_offset to the constructor.
			//
			// NOTE: Uncompressed bit-stream seem to have their BRAM data swapped at 32-bit
			// word-level. BRAM extract/insert functions must take care of the word swapping (if
			// needed).
			//
			//------------------------------------------------------------------------------------------
			//
			// Further reverse engineering on the data-bit and parity-bit offsets shows a quite regular
			// structure that can easily be described using two small lookup tables (16 entries each), a
			// constant (0xCA) and several shifts.
			//
			...
			*/
			static uint32_t ramb36e1_map_data_bit(const uint32_t data_offset)
			{
				...
				const uint32_t base_offset = (data_offset / 256u) * block_scale +
					groupH_table[data_offset & 0x0Fu];
			...
			*/
			static uint32_t ramb36e1_map_parity_bit(const uint32_t parity_offset)
			{
				...
				const uint32_t base_offset = (parity_offset / 32u) * block_scale +
					groupP_table[parity_offset & 0x01u];
