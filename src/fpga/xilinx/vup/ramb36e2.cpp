...
namespace fpga
{
	namespace xilinx
	{
		namespace vup
		{
			...
			//
			// The RAMB36E2 tiles work very similar to the Virtex-7 series tiles (e.g. the RAMB36E1 implementation in the v7
			// namespace).
			//
			// Mapping of bits in the bitstream can be determined (as for the Zynq-7 devices) by synthesizing a design that uses
			// all block RAMs and generating a bitstream with logic location information. Mapping tables in this file were inferred
			// in that way.
			//
			//--------------------------------------------------------------------------------------------------------------------
			/**
			...
			static uint32_t ramb36e2_map_data_bit(const uint32_t data_offset)
			{
				...
				static const uint32_t bit_table[] =
				{
					...
					0x47u, 0xCBu, 0x53u, 0xD7u, 0x5Fu, 0xE3u, 0x6Bu, 0xEF
				};
